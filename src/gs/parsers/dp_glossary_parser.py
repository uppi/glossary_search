        ...
        while True:
            if self.rownum < self.sheet.nrows:
                ...
                if key[:3] == "SYS" and key[-5:] == "_NAME":
                    key = key[3:-5]
                item_type = unicode(self.sheet.row_values(self.rownum)[1])
                data = unicode(self.sheet.row_values(self.rownum)[2])
                english = unicode(self.sheet.row_values(self.rownum)[3])
                ...
                self.rownum += 1
                if data:
                    result = Row([english, key, item_type, data, tr1, tr2], main_col=3)
                    return result
            else:
                return None
